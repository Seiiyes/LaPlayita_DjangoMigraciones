...
if [ ! -z "$DATABASE_URL" ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        TABLE_COUNT=$(mysql -h$DB_HOST -P$DB_PORT -u$DB_USER -p$DB_PASS -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='$DB_NAME';" -s -N $DB_NAME)
        
        if [ "$TABLE_COUNT" -eq 0 ] || [ "$TABLE_COUNT" -lt 10 ]; then
            echo "Base de datos vacía o incompleta. Importando backup..."
            mysql -h$DB_HOST -P$DB_PORT -u$DB_USER -p$DB_PASS $DB_NAME < /app/database/ult_ver_backup_912.sql
